"use strict";
const DYNAM_SHEET_PREFIX = 'dynamics_';
const STRING_DATE_REGEX = /^(\d{2})\.(\d{2})\.(\d{4}) (\d{2}):(\d{2}):(\d{2})$/;
function getDateFromTable(_Sheet) {
    let firstDateCol = null;
    const fullSecondRow = _Sheet.getRange(2, 1, 1, _Sheet.getLastColumn()).getValues();
    for (let i = 0; i < fullSecondRow[0].length; i++) {
...
function createDynamicsGraph(_dynamicsSheet) {
            ...
            if (parentSheet) {
                ...
                if (dateCol) {
                    if (parentSheet.getLastRow() - 1 > activeSheet.getLastRow() || parentSheet.getLastRow() - 1 < activeSheet.getLastRow()) {
                        const parentSheetData = parentSheet.getRange(2, dateCol, parentSheet.getLastRow() - 1, 1).getValues();
            ...
            if (childSheet) {
                ...
                if (dateCol) {
                    if (activeSheet.getLastRow() - 1 > childSheet.getLastRow() || activeSheet.getLastRow() - 1 < childSheet.getLastRow()) {
                        const activeSheetData = activeSheet.getRange(2, dateCol, activeSheet.getLastRow() - 1, 1).getValues();
