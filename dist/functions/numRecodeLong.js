"use strict";
function numRecodeLong() {
    const context = getSheetContext();
    if (!context)
        return;
    const { spreadsheet, ui, activeSheet } = context;
    try {
        const dataHeaders = activeSheet.getRange(1, 1, 1, activeSheet.getLastColumn()).getValues();
        const dataCols = ['Q', 'Option'];
        const dataIndices = dataCols.map(item => dataHeaders[0].indexOf(item));
        const dataQs = activeSheet.getRange(2, dataIndices[0] + 1, activeSheet.getLastRow()).getValues();
        const dataOpts = activeSheet.getRange(2, dataIndices[1] + 1, activeSheet.getLastRow()).getValues();
        if (dataQs.length != dataOpts.length)
            return ui.alert('Diff length on columns');
        const scaleSheet = spreadsheet.getSheetByName('scales');
        if (!scaleSheet)
            return ui.alert('No scales sheet found');
        const scaleHeaders = scaleSheet.getRange(1, 1, 1, scaleSheet.getLastColumn()).getValues();
        const scaleCols = ['Question', 'Option', 'Code'];
        const scaleIndices = scaleCols.map(item => scaleHeaders[0].indexOf(item));
        ...
        if (scaleQs.length != scaleOpts.length || scaleQs.length != scaleCodes.length)
            return ui.alert('Diff lenght on columns');
        const recodedValues = [];
