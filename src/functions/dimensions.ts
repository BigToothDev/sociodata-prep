function dimensions(): void | GoogleAppsScript.Base.Button {
    const context = getSheetContext();
    if (!context) return;
    const { ui, activeSheet } = context;
    try {
        const headers = activeSheet.getRange(1, 1, 1, activeSheet.getLastColumn()).getValues() as string[][];
        const q: number = headers[0].indexOf('Q');
        const qIndex: number = q + 1;
        const dataToDimension = activeSheet.getRange(2, qIndex, activeSheet.getLastRow() - 1, 1).getValues() as string[][];
        const firstDimension: string[] = [];
        const secondDimension: string[] = [];
        const qShort: string[] = [];
        for (let i = 0; i < dataToDimension.length; i++) {
            let value: string = dataToDimension[i][0];
            if (!value || value == '') {
                firstDimension.push('');
                secondDimension.push('');
                qShort.push('');
            } else {
                const squareBracketsMatch: RegExpMatchArray | null = value.match(/\[[^\]]*\]/g);
